...
执行模型、入口模型与工程结构整体收口，Aethe 从历史上的解释型多入口实验形态切到单文件编译器主线。

### 变更

- 主执行路径固定为 `source -> lexer/parser -> custom bytecode -> VM`
- 不再保留解释器回退路径，超出当前子集的语法在编译阶段直接失败
- 编译器新增字节码优化，当前包含常量折叠、简单 peephole 压缩、冗余跳转消除与不可达代码裁剪
- 工程结构重新收回单文件，编译优化逻辑内联回 `main.cpp`
- `CMakeLists.txt` 直接内联了编译优化选项与可用时的 IPO/LTO，不再依赖额外构建配置文件

### 语言与运行时收敛

- 语言层不再提供 `read_file`
- `type`、`match`、`for`、`defer` 明确退出当前编译子集
- 宿主 CLI 不再接受脚本文件路径，只保留终端 IDE、`stdin` 单次执行与 `--dump-bytecode`
- 沙盒环境下传入的位置参数会被忽略，不再作为脚本路径解释
- 终端 IDE 刻意裁成最小可用子集，只保留基础编辑、运行与输出查看

### 文档

- 更新 [README.md](/Users/armand/Personal/CLion/Aethe/README.md) 到 `Aethe 4 / 4.0.0`，同步当前单文件编译器定位
- 更新 [REFERENCE.md](/Users/armand/Personal/CLion/Aethe/REFERENCE.md) 的实现范围、版本号与当前编译子集说明
- 更新 [TUTORIAL.md](/Users/armand/Personal/CLion/Aethe/TUTORIAL.md) 的版本号与当前 IDE/运行方式描述
- 更新本文件，明确 `4.0.0` 是当前收口版本

### 验证

- 使用 `g++ -std=c++11 -DAETHE_ENABLE_THREADS=1 -pthread main.cpp -o /tmp/aethe-bytecode-test` 编译通过
- 回归验证了 `stdin` 执行、`--dump-bytecode` 导出、自定义字节码优化、常量条件分支裁剪
- 验证了 `type` 现在会在编译阶段直接报错
