...
这份教程对应 `Aethe 4 / 4.0.0`。目标不是只让你"看懂例子"，而是让你能够独立写出一段可运行的 Aethe 程序，并知道什么时候该使用 `fn`、什么时候该使用 `stream`（兼容 `stage`）、什么时候该使用匿名 `pipe`、什么时候该使用管道组合子、什么时候该写管道表达式。
...
默认启动后会进入一个全屏的终端编辑器（IDE 模式）。你可以在里面写代码、运行代码，并查看输出结果。
...
当前 IDE 刻意只保留最基础的编辑、运行和输出查看能力，方便把实现重心放在编译器和字节码虚拟机本身。

### 2.3 标准输入模式

如果你不想进 IDE，而是想在沙盒或脚本管道里一次性执行源码，可以把代码从标准输入喂给 Aethe：

```bash
cat example.ae | ./aethe --stdin
```

如果已经是非交互终端，也可以直接省略 `--stdin`：

```bash
cat example.ae | ./aethe
```

这两种写法都会读取完整输入、编译成字节码并执行一次。

### 2.4 查看字节码

如果你想看编译器生成的自定义字节码，可以这样做：
...
不管你在 IDE 里还是通过 `stdin` 执行，先写下面这行：

```scala
"hello, Aethe" |> emit;
```

如果你走 `stdin` 模式，可以直接这样试：

```bash
printf '"hello, Aethe" |> emit;\n' | ./aethe --stdin
```
...
- **`|?`**——安全管道，运行时错误自动包装为 `Err`，不会崩溃（见第 19 节）
...
如果中间某一步出了运行时错误（比如 `parse_json` 遇到非法 JSON），整个程序就崩掉了。你要么到处加 `when` 判断，要么整条管道都战战兢兢。
...
1. 如果 `parse_json` 出错，错误会被自动捕获，包装成 `Err("错误信息")`
2. 后面的 `|?` 步骤检测到是 `Err`，就直接跳过，继续往下传
3. 最终 `$result` 拿到的是一个 `Err(...)`，而不是崩溃

如果一切顺利，每个 `|?` 步骤都正常执行，最终 `$result` 拿到的是 `Ok(最终值)`。

### 19.3 `Ok` 和 `Err`：结果类型

`|?` 管道的结果是以下两种之一：

- `Ok(value)`——整条管道都成功执行，`value` 是最终值
- `Err(message)`——某个步骤出了错，`message` 是错误描述
...
如果输入是合法整数，输出 `计算结果：84`；如果转换失败，输出 `出错了：...`。

### 19.4 `|>` 和 `|?` 可以混用

你不必把整条管道都改成 `|?`。只有你认为可能出错的部分才需要 `|?`：

```scala
// 前半段是安全操作，用普通 |>
let data = [1, 2, 3, 4, 5]
    |> map(bind(add, 10))
    |> filter(pipe(x) { return $x > 12; });
...
match $result {
    case Ok(v) { $v |> emit; }
    case Err(e) { "error: " |> concat($e) |> emit; }
}
```

### 19.5 什么时候用 `|?`

- 处理**用户输入**：输入格式不可控，随时可能不合法
- **文件读取后解析**：文件内容不保证格式正确
- **数学运算**：可能除零
- **长管道**：某些步骤失败不希望整个程序崩溃

不需要用的场景：数据是已知安全的、你愿意让错误直接报出来时。
...
要求：写一段代码，用 `|?` 管道对用户输入的字符串做 `int` 转换后乘以 2。如果转换失败，输出错误信息而不崩溃。
...
let result = $raw |? int |? pipe(x) { return $x * 2; };

match $result {
    case Ok(v) { "结果：" |> concat($v) |> emit; }
    case Err(e) { "失败：" |> concat($e) |> emit; }
}
```
